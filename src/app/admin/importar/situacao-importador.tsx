...
import { previewSituacaoVR, confirmarSituacaoVR, type PreviewSituacaoResult } from "./situacao-actions";
import { StatCard } from "./stat-card";
import { lerArquivoTexto } from "./ler-arquivo";

export function SituacaoImportador({
  competencia,
  onImportado,
}: {
  competencia: string;
  ...
  const [preview, setPreview] = useState<PreviewSituacaoResult | null>(null);
  ...
  async function handleFile(f: File) {
    ...
    startTransition(async () => {
      const result = await previewSituacaoVR(texto);
      if (result.error) {
  ...
  function handleConfirmar() {
    ...
    startTransition(async () => {
      const result = await confirmarSituacaoVR(conteudo, file.name, competencia, resolucoes);
      if (result.error) {
        toast.error(result.error);
        return;
      }
      toast.success(
        `Situação/VR aplicado: ${result.resumo?.atualizados} funcionário(s) atualizado(s).`
      );
      handleCancelar();
      onImportado?.();
    });
      ...
      <div>
        ...
        >
          ...
          <strong className="text-xs font-medium text-foreground">
            {file ? "Arquivo carregado" : "Clique para selecionar o relatório de situação/vale-refeição (.TXT)"}
          </strong>
        ...
        <>
          <div>
            ...
            <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-5">
              ...
              <StatCard label="Cargos VR" value={preview.cargosVR.length} cor="blue" />
              <StatCard label="Não encontrados" value={preview.naoEncontrados.length} cor="red" />
            </div>
          </div>

          {preview.funcoesNaoCadastradas.length > 0 && (
            <div className="flex flex-col gap-2">
              <p className="font-mono text-[10px] tracking-[.06em] text-warning uppercase">
                Funções do arquivo sem cargo cadastrado (elegibilidade de VR não sincronizada)
              </p>
              <div className="flex flex-wrap gap-1.5">
                {preview.funcoesNaoCadastradas.map((f) => (
                  <span
                    key={f}
          ...
          {preview.cargosVR.length > 0 && (
            <Table containerClassName="max-h-[40vh] overflow-y-auto">
              <TableHeader>
                <TableRow>
                  <TableHead>Cargo</TableHead>
                  <TableHead>Vale-refeição atual</TableHead>
                  <TableHead>Novo valor</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {preview.cargosVR.map((c) => (
                  <TableRow key={c.cargoId}>
                    <TableCell className="font-medium text-foreground">{c.nome}</TableCell>
                    <TableCell>{c.atual ? "Recebe" : "Não recebe"}</TableCell>
                    <TableCell>{c.novo ? "Recebe" : "Não recebe"}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
